...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  scheduled: { label: 'E planifikuar', color: 'bg-blue-100 text-blue-700' },
  in_progress: { label: 'Në vazhdim', color: 'bg-amber-100 text-amber-700' },
  completed: { label: 'E përfunduar', color: 'bg-emerald-100 text-emerald-700' },
  cancelled: { label: 'Anuluar', color: 'bg-slate-100 text-slate-600' },
};
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-orange-600 to-rose-600 rounded-2xl p-6 text-white">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Inspektorati i Arsimit</h1>
              <p className="text-orange-100 text-sm">Mirë se erdhët, {profile?.full_name}</p>
            </div>
          </div>
          <Link
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <StatCard icon={ClipboardCheck} label="Inspektime gjithsej" value={stats?.totalInspections || 0} color="orange" />
        <StatCard icon={Clock} label="Në pritje" value={stats?.pendingInspections || 0} color="amber" />
        <StatCard icon={CheckCircle} label="Të përfunduara" value={stats?.completedInspections || 0} color="emerald" />
        <StatCard icon={AlertTriangle} label="Gjetje aktive" value={stats?.openFindings || 0} color="rose" />
        <StatCard icon={School} label="Shkolla" value={stats?.schools || 0} color="blue" />
        <StatCard icon={Clock} label="Këtë muaj" value={stats?.thisMonthInspections || 0} color="cyan" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <Link to="/inspektor/inspektimet" className="bg-white rounded-2xl border border-slate-100 p-4 hover:shadow-lg hover:border-orange-200 transition-all group">
          ...
          <h3 className="font-semibold text-slate-900">Të gjitha inspektimet</h3>
          <p className="text-xs text-slate-500 mt-1">Menaxhim i inspektimeve të hapura</p>
        </Link>
        <Link to="/inspektor/shkollat" className="bg-white rounded-2xl border border-slate-100 p-4 hover:shadow-lg hover:border-blue-200 transition-all group">
          ...
          <h3 className="font-semibold text-slate-900">Shkollat e Kosovës</h3>
          <p className="text-xs text-slate-500 mt-1">Lista e plotë e 102 shkollave</p>
        </Link>
        <Link to="/inspektor/mesazhet" className="bg-white rounded-2xl border border-slate-100 p-4 hover:shadow-lg hover:border-teal-200 transition-all group">
          ...
          <h3 className="font-semibold text-slate-900">Mesazhet</h3>
          <p className="text-xs text-slate-500 mt-1">Komunikim me DKA/MAShTI</p>
        </Link>
      </div>
    </div>
  );
